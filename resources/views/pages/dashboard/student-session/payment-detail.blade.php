...
<div class="section-header">
  <h1>Detail Pembayaran SPP - <span class="text-capitalize">{{ $invoice->bill->month }}</span> {{ $invoice->bill->year
    }}</h1>

  <div class="section-header-breadcrumb">
    <a href="{{ route('student.payment-info') }}" class="btn btn-sm btn-icon icon-left btn-primary">
            ...
            <table class="table table-striped table-md">
              ...
              <tbody>
                ...
                <tr>
                  <td colspan="2"></td>
                  <td>
                    <a href="{{ $invoice->invoice_url }}" target="_blank" class="btn btn-icon icon-left btn-primary"
                      style="display:{{ $invoice->status == 'PENDING' ? ' ' : 'none' }};"><i
                        class="far fa-credit-card"></i> Bayar</a>
                  </td>
                </tr>
                @endif
              </tbody>
